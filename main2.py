...
class PersonalizedTutor:
    ...
    def generate_response(self,
        ...
        # Convert the prompt into a message list before invoking
    ...
    def generate_bar_chart(self, categories: List[str], values: List[int], title: str):
    ...
    def generate_pie_chart(self, categories: List[str], values: List[int], title: str):
    ...
    def generate_number_line(self, start: int, end: int, highlight: List[int] = []):
        """Generate a number line"""
        plt.figure(figsize=(8, 2))
        numbers = np.arange(start, end + 1, 1)
        plt.xticks(numbers)
        plt.yticks([])
        plt.xlim(start - 1, end + 1)
        plt.ylim(-1, 1)
        for num in numbers:
            plt.scatter(num, 0, color='black', s=50)
            plt.text(num, 0.2, str(num), fontsize=12, ha='center')
        for h in highlight:
            plt.scatter(h, 0, color='red', s=100)
        plt.title("Number Line Representation")
        plt.show()
...
student = StudentProfile(
    ...
    previous_topics=["Basic Algebra", "Fractions"],
    struggles=["Word Problems"],
    strengths=["Number Operations"]
)

# Initialize tutor with your API key
tutor = PersonalizedTutor(api_key="your-api-key-here")
tutor.generate_bar_chart(["Math", "Science", "History", "English"], [80, 75, 60, 70], "Student Grades")
tutor.generate_pie_chart(["Math", "Science", "History", "English"], [80, 75, 60, 70], "Student Grades")
tutor.generate_number_line(1, 10, highlight=[5])